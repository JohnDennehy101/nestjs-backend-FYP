  ...
  async createEvent(
    ...
    try {
      const newEvent = await this.create({
        ...eventDto,
        cityLatitude: cityCoordinates[0],
        cityLongitude: cityCoordinates[1],
        createdByUser: user,
  ...
  async findAllUserInvitedEvents(user: User): Promise<Event[]> {
    try {
      const events = await this.createQueryBuilder('event')
        .leftJoin('event.invitedUsers', 'event_invited_users')
        .select(['event'])
        .where('event_invited_users.id = :id', { id: user.id })
        .getMany();
      return events;
    } catch (error) {
      console.log(error);
  ...
  async findEvent(uuid: string): Promise<any> {
    try {
      const event = await this.createQueryBuilder('event')
        .leftJoinAndSelect('event.polls', 'poll')
        .where('event.id = :id', { id: uuid })
        .leftJoinAndSelect('poll.pollOptions', 'poll_option')
        .leftJoinAndSelect('poll_option.votes', 'poll_vote')
        .leftJoinAndSelect('event.createdByUser', 'user')
        .where('event.id = :id', { id: uuid })
        .leftJoinAndSelect('event.invitedUsers', 'event_invited_users')
        .select([
          'event',
          'poll',
          'user.id',
  ...
  async findEventUsers(uuid: string): Promise<any> {
    try {
      const event = await this.createQueryBuilder('event')
        ...
        .where('event.id = :id', { id: uuid })
        .leftJoinAndSelect('event.invitedUsers', 'event_invited_users')
        .select(['event', 'poll', 'event_invited_users'])
        .where('event.id = :id')
        .getMany();
      return event;
    } catch (error) {
