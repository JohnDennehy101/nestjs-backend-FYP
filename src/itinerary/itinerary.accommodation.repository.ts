...
export class ItineraryAccommodationRepository extends Repository<ItineraryAccommodation> {
  async updateItineraryAccommodation(
      ...
      if (
        accommodation.filter(
          (individualAccommodation) =>
            individualAccommodation.title ===
              previousAccommodation[item].title ||
            individualAccommodation.price ===
              previousAccommodation[item].price ||
            individualAccommodation.reviewQuantity ===
              previousAccommodation[item].reviewQuantity,
        ).length === 0
      ) {
    ...
    for (let item in accommodation) {
      ...
      if (existingRecordCheck.length > 0) {
        await this.update(existingRecordCheck[0].id, {
          ...
          ...(accommodation[item].bookingPreviewLink && {
          ...
          ...(accommodation[item].ratingScore && {
            ratingScore: accommodation[item].ratingScore,
          }),
          ...(accommodation[item].ratingScoreCategory && {
            ratingScoreCategory: accommodation[item].ratingScoreCategory,
          }),
          ...(accommodation[item].reviewQuantity && {
          ...
          ...(accommodation[item].startDate && {
            startDate: accommodation[item].startDate,
          }),
          ...(accommodation[item].endDate && {
            endDate: accommodation[item].endDate,
          }),
          ...(accommodation[item].locationTitle && {
